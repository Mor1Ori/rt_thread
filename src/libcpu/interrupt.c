...
{
    // 计算中断对应的位索引
    int bit_index = vector % 32;

    // 读取当前 mie 寄存器的值
    unsigned int mie_value;
    asm volatile ("csrr %0, mstatus" : "=r"(mie_value));

    // 设置对应中断位为 0，即屏蔽对应中断
    mie_value &= ~(1 << bit_index);

    // 将更新后的值写回 mie 寄存器
    asm volatile ("csrw mstatus, %0" :: "r"(mie_value));
    //rt_hw_plic_irq_disable(0, vector);
}
...
{
    unsigned int mie_value=0x00000080;
    //asm volatile ("csrr %0, mie" : "=r"(mie_value));
    //asm volatile ("csrw mstatus, %0" :: "r"(mie_value));
    asm volatile ("csrw mie, %0" :: "r"(mie_value));
}
void rt_hw_interrupt_maskALL()
{
    unsigned int mie_value=0x00000000;
    //asm volatile ("csrr %0, mie" : "=r"(mie_value));
    //asm volatile ("csrw mstatus, %0" :: "r"(mie_value));
    asm volatile ("csrw mie, %0" :: "r"(mie_value));
}
