...
{
  // 设置 LED 显示进入函数
  WRITE_GPIO(GPIO_LEDs, 0x00000001);
  
  if (ch == '\n') {
    WRITE_GPIO(GPIO_LEDs, 0x00000002);  // 指示处理换行
    printUartPutchar('\r');
  }

  // 指示等待 UART
  WRITE_GPIO(GPIO_LEDs, 0x00000004);
  
  /* 添加超时 */
  volatile unsigned int timeout = 100000;
  while(((M_UART_RD_REG_LSR() & D_UART_LSR_THRE_BIT) == 0) && timeout > 0) {
    timeout--;
  }
  
  // 指示发送字符
  WRITE_GPIO(GPIO_LEDs, 0x00000008);
  M_UART_WR_CH(ch);
